...
const HeroCarousel = () => {
  ...
  return (
    <div className="relative w-full h-[300px] md:h-[500px] rounded-[40px] overflow-hidden shadow-2xl border-[12px] border-white/10 group animate-scale-in">
      {HERO_IMAGES.map((img, idx) => (
        ...
        >
          <SafeImage
            ...
            className={`w-full h-full object-cover ${idx === currentIndex ? 'animate-image-zoom' : ''}`}
          />
...
const QuickLinkCard = ({ icon: Icon, title, desc, link, index }: any) => (
  <Link
    to={link}
    className="bg-white p-8 rounded-3xl shadow-sm hover:shadow-xl transition-all group border border-gray-100 hover:border-brand-gold/30 animate-fade-in-up"
    style={{ animationDelay: `${index * 150}ms` }}
  >
    <div className="w-14 h-14 bg-brand-soft rounded-2xl flex items-center justify-center text-brand-teal mb-6 group-hover:bg-brand-teal group-hover:text-white transition-all duration-300 transform group-hover:rotate-6">
      <Icon size={28} />
    </div>
  ...
  return (
    <div className="overflow-hidden">
      ...
      <section className="relative min-h-[700px] py-20 flex items-center bg-brand-teal overflow-hidden">
        ...
        <div className="absolute top-0 right-0 w-1/2 h-full bg-brand-lightTeal/20 -skew-x-12 translate-x-1/4 animate-fade-in"></div>
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-brand-gold/10 rounded-full blur-3xl animate-pulse"></div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="relative animate-slide-in-right">
              <HeroCarousel />
              {/* Floating Badge */}
              <div className="absolute -bottom-6 -left-6 bg-white p-6 rounded-[32px] shadow-2xl hidden md:block animate-bounce-slow">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
            <div className="order-2 lg:order-1 relative animate-fade-in-up">
              <div className="rounded-[40px] overflow-hidden shadow-2xl relative z-10 aspect-[4/5] group">
                <SafeImage
              ...
              <div className="absolute -bottom-10 -right-10 bg-brand-gold p-10 rounded-[32px] shadow-2xl z-20 hidden md:block border-[12px] border-white animate-scale-in delay-300">
            ...
            <div className="order-1 lg:order-2 animate-slide-in-right">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {['English-Medium Instruction', 'FBISE & IGCSE Curriculums', 'Values-Based Learning', 'Community-Driven Model'].map((item, idx) => (
                  <div key={item} className="flex items-center space-x-3 p-4 bg-brand-soft/50 rounded-2xl border border-brand-teal/5 animate-fade-in-up" style={{ animationDelay: `${idx * 100}ms` }}>
      ...
      <section className="py-28 bg-brand-soft/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
            ...
            <Link to="/contact" className="text-brand-teal font-bold flex items-center group transition-transform hover:translate-x-2">
              View All Resources <ArrowRight size={20} className="ml-2 group-hover:translate-x-2 transition-transform" />
            </Link>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              { icon: UserPlus, title: "Admissions", desc: "Step-by-step guidance for new registrations and transfers.", link: "/admissions" },
              { icon: CreditCard, title: "Fees Structure", desc: "Details on our community-focused and affordable fee model.", link: "/admissions" },
              { icon: Clock, title: "Timings", desc: "Daily schedules for all school sections and divisions.", link: "/info" },
              { icon: BookOpen, title: "Syllabus", desc: "Access curriculum planners for all academic grades.", link: "/academics" },
              { icon: Calendar, title: "Calendar", desc: "Stay updated with exams, holidays, and school events.", link: "/info" },
              { icon: ShoppingBag, title: "Bookstore", desc: "Online purchase for stationery and textbooks.", link: "/store" }
            ].map((card, idx) => (
              <QuickLinkCard
                key={card.title}
                icon={card.icon}
      ...
      {/* Final CTA */}
      <section className="py-20 bg-brand-gold overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col lg:flex-row items-center justify-between text-center lg:text-left gap-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto animate-slide-in-right">
            <Link to="/admissions" className="bg-brand-teal text-white px-10 py-5 rounded-2xl font-bold hover:bg-brand-lightTeal transition-all shadow-xl text-center text-lg whitespace-nowrap active:scale-95">
              Start Application
            </Link>
            <Link to="/contact" className="bg-white text-brand-teal px-10 py-5 rounded-2xl font-bold hover:bg-gray-50 transition-all shadow-md text-center text-lg whitespace-nowrap active:scale-95">
              Visit Campus
            </Link>
          </div>
        </div>
